...
describe('inspector workspace resources', () => {
  ...
  it('reads the selected project without a session and keeps an in-flight preview across session transitions', async () => {
    const pending = deferred()
    const read = vi.fn(() => pending.promise)
    const controller = new InspectorResourcesController(workspaceId, read)
    expect(controller.open('a.ts')).toBe(true)
    await settle()
    controller.setSession('a')
    controller.setSession(null)
    controller.setSession('b')
    pending.resolve(preview('a.ts', 'project content'))
    await settle()
    expect(read).toHaveBeenCalledTimes(1)
    expect(controller.getSnapshot().files[0]).toMatchObject({ phase: 'ready', preview: { content: 'project content' } })
  })

  it('coalesces background reads without changing the selected resource and reuses an unchanged preview', async () => {
    const pending = deferred()
    const original = preview('a.ts')
    const read = vi.fn().mockResolvedValueOnce(original).mockReturnValueOnce(pending.promise)
    const controller = new InspectorResourcesController(workspaceId, read)
    controller.open('a.ts')
    await settle()
    const first = controller.refreshActive()
    expect(controller.refreshActive()).toBe(first)
    controller.showTree()
    pending.resolve({ ...original })
    await first
    expect(read).toHaveBeenCalledTimes(2)
    expect(controller.getSnapshot().activePath).toBeNull()
    expect(controller.getSnapshot().files[0].preview).toBe(original)
