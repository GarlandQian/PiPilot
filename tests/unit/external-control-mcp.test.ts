...
describe('official conversation MCP server', () => {
  ...
  it('requires one absolute descriptor path and fails unavailable without stdout', async () => {
    const defaultDescriptor = join(temporaryDirectory(), 'descriptor.json')
    expect(parseConversationMcpDescriptorPath([], defaultDescriptor)).toBe(defaultDescriptor)
    ...
    expect(() => parseConversationMcpDescriptorPath([], 'relative.json'))
      .toThrowError(expect.objectContaining({ code: 'invalid_state' }))
    expect(() => parseConversationMcpDescriptorPath([
      '--descriptor',
      'relative.json',
