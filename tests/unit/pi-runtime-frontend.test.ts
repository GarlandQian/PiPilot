...
class FakeFrontendPool {
  ...
  async createRuntime(
    ...
    const sessionFile = this.createWithoutSessionFile
      ? null
      : target.sessionFile ?? `/sessions/${runtimeId}.jsonl`
    this.runtime = {
      runtimeId,
      generation: 1,
      cwd: scope.cwd,
    ...
    this.runtimeStates.set(runtimeId, {
      ...
      state: sessionState(this.runtime.sessionId, sessionFile ?? undefined),
    })
...
describe('PiRuntimeFrontend', () => {
  it('projects live session status over a retained terminal failure on recovery', () => {
    const failed: LocalPiRuntimeSessionStatus = {
      scope: projectScope,
    ...
    expect(projectRuntimeSessionStatuses([], [failed])).toEqual([failed])
    expect(projectRuntimeSessionStatuses([summary], [failed])).toEqual([{
      scope: projectScope,
      sessionId: 'session-recovered',
      status: 'completed',
    }])
  ...
  it('marks queued and active runtimes as running without exposing paths', () => {
    const selectionToken = `sel_${'r'.repeat(32)}` as const
    const summary = {
      hostEpoch: 1,
      runtimeId: 'runtime-running',
      generation: 1,
      scope: projectScope,
      sessionFile: '/private/should-not-cross-ipc.jsonl',
    ...
    const [status] = projectRuntimeSessionStatuses([summary], [])
  ...
  it('bounds retained status history without dropping a live Runtime', () => {
    const retained = Array.from(
      { length: LOCAL_PI_RUNTIME_SESSION_STATUS_MAX_ITEMS + 1 },
      (_, index): LocalPiRuntimeSessionStatus => ({
        scope: projectScope,
    ...
    const statuses = projectRuntimeSessionStatuses([live], retained)

    expect(statuses).toHaveLength(LOCAL_PI_RUNTIME_SESSION_STATUS_MAX_ITEMS)
  ...
  it('publishes failed assistant outcomes without republishing token deltas', async () => {
    const { frontend, pool } = createHarness()
    const selectionToken = `sel_${'f'.repeat(32)}` as const
    ...
    pool.emitEvent(runtimeEvent(runtime.runtimeId, runtime.generation, {
      type: 'message_update',
      usage: {
        input: 0,
        output: 1,
    ...
    expect(frontend.getSnapshot().sessionStatuses).toContainEqual({
      scope: projectScope,
      sessionId: 'session-1',
      selectionToken,
      status: 'failed',
  ...
  it('acquires and submits to a background Runtime without changing selection', async () => {
    ...
    const { sessionStatuses: _selectedStatuses, ...selectedSnapshotWithoutStatuses } = selectedSnapshot
    const currentSnapshot = frontend.getSnapshot()
    const { sessionStatuses: _currentStatuses, ...currentSnapshotWithoutStatuses } = currentSnapshot
    expect(currentSnapshotWithoutStatuses).toEqual(selectedSnapshotWithoutStatuses)
    expect(currentSnapshot.sessionStatuses).toEqual(expect.arrayContaining([
    ...
    expect(frontend.getActiveRuntimeIdentity()).toEqual(selected)
    const afterSubmit = frontend.getSnapshot()
    const {
      sessionStatuses: _afterSubmitStatuses,
      ...afterSubmitWithoutStatuses
    } = afterSubmit
    const {
      sessionStatuses: _selectedStatusesAfterSubmit,
      ...selectedSnapshotWithoutStatusesAfterSubmit
    } = selectedSnapshot
    expect(afterSubmitWithoutStatuses).toEqual(selectedSnapshotWithoutStatusesAfterSubmit)
    expect(afterSubmit.sessionStatuses).toEqual(expect.arrayContaining([
      expect.objectContaining({ sessionId: 'session-1', status: 'completed' }),
  ...
  it('does not forward active blocking UI after a Main consumer cancels it', async () => {
    ...
    frontend.subscribe((snapshot) => {
      publishedStatuses.push(snapshot.sessionStatuses)
    })
    frontend.subscribeUiRequests((event) => {
      selectedUi.push(event.request.id)
    })
  ...
  it('hard-recovers the same Session when an abort cannot reach a crashed Host', async () => {
    const { frontend, pool } = createHarness()
    const sessionFile = '/sessions/stuck-tool.jsonl'
    await frontend.start({ scope: projectScope, sessionFile })
    const firstIdentity = frontend.getActiveRuntimeIdentity()
    pool.emitSnapshot({
      state: 'ready',
      hosts: [pool.hostSnapshot(
        { kind: 'project', cwd: projectCwd },
        'crashed',
        pool.runtime,
      )],
    })
    expect(frontend.getSnapshot().state).toBe('crashed')
    pool.abortError = new ProjectHostPoolError(
      'HOST_CRASHED',
      'The stuck Host crossed the abort grace boundary.',
    )

    await expect(frontend.request({ type: 'abort' })).resolves.toEqual({
      type: 'response',
      command: 'abort',
      success: true,
    })
    ...
    expect(pool.commandCalls).toContainEqual(expect.objectContaining({
      runtimeId: firstIdentity?.runtimeId,
      command: { type: 'abort' },
      timeoutMs: PI_RUNTIME_ABORT_GRACE_TIMEOUT_MS,
    }))
    expect(pool.restartCalls).toEqual([{ kind: 'project', cwd: projectCwd }])
    expect(pool.createCount).toBe(2)
    ...
    expect(frontend.getActiveRuntimeIdentity()?.runtimeId).not.toBe(
    ...
    expect(pool.commandCalls.filter(({ command }) => command.type === 'prompt')).toHaveLength(0)
    await frontend.dispose()
  }, 1_000)

  it('recovers when Utility reports abort timeout before Host failure arrives', async () => {
    const { frontend, pool } = createHarness()
    const sessionFile = '/sessions/abort-timeout.jsonl'
    await frontend.start({ scope: projectScope, sessionFile })
    pool.abortError = new PiHostControllerError(
      'REQUEST_FAILED',
      'Runtime abort did not finish within its grace period.',
      true,
      {
        name: 'RuntimeManagerError',
        message: 'Runtime abort timed out.',
        code: 'RUNTIME_OPERATION_TIMEOUT',
      },
    )

    await expect(frontend.request({ type: 'abort' })).resolves.toMatchObject({
      command: 'abort',
      success: true,
    })
  ...
  it('fails closed when a timed-out abort has no persisted Session to recover', async () => {
    ...
    await expect(frontend.request({ type: 'abort' })).rejects.toMatchObject({
      code: 'PI_RUNTIME_HOST_RECOVERY_FAILED',
      recoverable: false,
    })

    expect(pool.restartCalls).toHaveLength(0)
    expect(pool.createCount).toBe(1)
    expect(pool.commandCalls.filter(({ command }) => command.type === 'prompt')).toHaveLength(0)
    await frontend.dispose()
  }, 1_000)
  ...
  it('drops and acknowledges extension UI from a Runtime that became stale before projection', async () => {
    const { frontend, pool } = createHarness()
    await frontend.start({ scope: projectScope })
    const runtime = pool.runtime!
    const selectedUi: string[] = []
    ...
    frontend.subscribeAllUiRequests((event) => {
      allUi.push(event.request.id)
    })

    const host = pool.hostSnapshot(
      { kind: 'project', cwd: projectCwd },
      'ready',
      runtime,
    )
    host.runtimes[0]!.state = 'stopping'
    pool.emitSnapshot({ state: 'ready', hosts: [host] })

    const acknowledgementsBefore = pool.acknowledgedEvents.length
    pool.emitUiRequest(uiRequest(
      runtime.runtimeId,
      runtime.generation,
      'late-confirm',
    ...
    await vi.waitFor(() => {
      expect(pool.acknowledgedEvents).toHaveLength(acknowledgementsBefore + 1)
    })
    expect(pool.acknowledgedEvents[pool.acknowledgedEvents.length - 1]).toMatchObject({
      kind: 'ui_request',
      runtimeId: runtime.runtimeId,
      runtimeGeneration: runtime.generation,
      request: { id: 'late-confirm' },
    })
    expect(selectedUi).toEqual([])
    expect(allUi).toEqual([])
    await frontend.dispose()
  })

  it('drops and acknowledges late extension UI after its active Runtime is removed', async () => {
    const { frontend, pool } = createHarness()
    await frontend.start({ scope: projectScope })
    const runtime = pool.runtime!
    const selectedUi: string[] = []
    ...
    expect(pool.acknowledgedEvents[acknowledgementsBefore]).toMatchObject({
      kind: 'ui_request',
      runtimeId: runtime.runtimeId,
      runtimeGeneration: runtime.generation,
      request: { id: 'late-after-removal' },
    })
    expect(selectedUi).toEqual([])
    expect(allUi).toEqual([])
    await frontend.dispose()
  ...
  it('drops and acknowledges late events after their active Runtime is removed', async () => {
    const { frontend, pool } = createHarness()
    await frontend.start({ scope: projectScope })
    const runtime = pool.runtime!
    const selectedEvents: string[] = []
    ...
    frontend.subscribeAllEvents((event) => {
  ...
  it('retains failed status for background Runtimes when their shared Host crashes', async () => {
    const { frontend, pool } = createHarness()
    await frontend.start({
      scope: projectScope,
      sessionFile: '/sessions/selected.jsonl',
    ...
    pool.emitSnapshot({
      ...
      hosts: [pool.hostSnapshot(
        ...
        'crashed',
        [selected, {
          runtimeId: background.runtimeId,
    ...
    expect(frontend.getSnapshot().sessionStatuses).toEqual(expect.arrayContaining([
      expect.objectContaining({ sessionId: selected.sessionId, status: 'failed' }),
      expect.objectContaining({ sessionId: background.sessionId, status: 'failed' }),
    ]))
    await frontend.dispose()
  })
