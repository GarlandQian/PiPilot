...
test('runs the bundled Pi SDK workflow from the packaged application', async () => {
  ...
  try {
    ...
    // A fatal extension shutdown must become a terminal Host state. The next
    // explicit persisted-Session click creates one fresh Host and hydrates its
    // history instead of leaving every row permanently unavailable.
...
test('runs the copied packaged MCP command headlessly through the private bridge', async () => {
  test.setTimeout(180_000)
  const executable = resolvePackagedExecutable()
  const userDataPath = await mkdtemp(join(tmpdir(), 'pipilot-packaged-mcp-'))
  ...
  try {
    ...
    const expectedMcpExecutable = process.platform === 'win32'
      ? join(dirname(executable), 'PiPilot-mcp.exe')
      : executable
    const expectedMcpEntry = process.platform === 'win32'
      ? join(dirname(executable), 'resources', 'app.asar', 'out', 'main', 'index.js')
      : undefined
    expect(configuration).toMatchObject({
      command: expectedMcpExecutable,
      args: process.platform === 'win32'
        ? [expectedMcpEntry, '--pipilot-mcp-stdio', '--descriptor', expect.any(String)]
        : ['--pipilot-mcp-stdio', '--descriptor', expect.any(String)],
      ...(process.platform === 'win32'
        ? { env: { ELECTRON_RUN_AS_NODE: '1' } }
        : {}),
    })
    const descriptorArgumentIndex = process.platform === 'win32' ? 3 : 2
    expect(await realpath(configuration!.args[descriptorArgumentIndex]!)).toBe(join(
    ...
    const descriptorPath = configuration!.args[descriptorArgumentIndex]!
    ...
    const waitForMessage = async (id: number) => {
      ...
      // handshake a bounded 30-second startup window.
