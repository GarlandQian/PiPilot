...
test('recovers a projectless Pi Host after a one-shot fatal extension shutdown', async ({}, testInfo) => {
  ...
  try {
    ...
    const recovered = await page.evaluate(async () => {
      await window.pipilot!.conversation.new({ kind: 'projectless' })
      return window.pipilot!.localPi.runtime.status()
    })
...
test('navigates from the conversation outline to the exact visible turn', async ({}, testInfo) => {
  ...
  try {
    ...
    await transcriptLog.evaluate((element, expectedResponse) => {
      ...
      const probe = {
        ...
        sawPartialTyping: false,
        sawTyping: false,
      }
      const sample = () => {
        if (host.getAttribute('data-transcript-typing') !== 'true') return
        probe.sawTyping = true
        if (!host.textContent?.includes(expectedResponse)) probe.sawPartialTyping = true
      }
    ...
    await page.getByRole('button', { name: 'Send', exact: true }).click()
    await expect(transcriptLog).toHaveAttribute('data-transcript-typing', 'true', {
      timeout: 20_000,
    })
    ...
    const typewriterProbe = await transcriptLog.evaluate((element) => {
      ...
      probe?.observer.disconnect()
      return {
        sawPartialTyping: probe?.sawPartialTyping ?? false,
        sawTyping: probe?.sawTyping ?? false,
      }
    })
    expect(typewriterProbe).toEqual({
      sawPartialTyping: true,
      sawTyping: true,
    })
