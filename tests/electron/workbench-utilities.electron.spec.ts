...
test('preserves file reading and PTY identity while navigating and refreshing workbench tools', async ({}, testInfo) => {
  ...
  try {
    ...
    await fileRow.click()
    await expect(page.getByRole('tab', { name: 'src/nested/entry.ts', exact: true })).toHaveAttribute('aria-selected', 'true')
    await page.getByRole('tab', { name: 'Working tree', exact: true }).click()
    ...
    const terminal = page.locator('[data-terminal-status]')
    ...
    await terminal.getByRole('button', { name: 'Clear terminal display', exact: true }).click()
    const afterClear = await page.evaluate(async (scope) => window.pipilot!.terminal.create(scope, 80, 24), before.scope)
    expect(afterClear.terminalId).toBe(before.terminalId)
    expect(afterClear.reused).toBe(true)
    const command = terminal.getByRole('textbox', { name: 'Terminal input', exact: true })
    await command.fill('exit 0')
    ...
    await expect(terminal).toHaveAttribute('data-terminal-status', 'running')
    const afterExit = await page.evaluate(async (scope) => window.pipilot!.terminal.create(scope, 80, 24), before.scope)
