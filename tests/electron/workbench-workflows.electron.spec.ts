...
test('connects project actions, conversation navigation and resource views without stopping background work', async ({}, testInfo) => {
  ...
  try {
    ...
    await expect(page.getByRole('button', { name: 'Workspace review', exact: true })).toBeVisible()
    ...
    await expect(page.getByRole('button', { name: 'Workspace review', exact: true })).toHaveCount(0)
    ...
    await page.getByRole('button', { name: 'Workspace review', exact: true }).click()
