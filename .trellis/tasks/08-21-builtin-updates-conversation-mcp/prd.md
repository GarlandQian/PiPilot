...
- Publish the same copyable client shape on every platform: exactly
  `{ command, args }`, with args
  `['--pipilot-mcp-stdio', '--descriptor', absoluteDescriptorPath]`. The
  validated absolute command and descriptor values may differ by installation;
  clients must not receive a Windows-only ASAR argument, environment override,
  PATH lookup, or home-directory placeholder.
...
- [x] Main/shared/Renderer publish one strict `{ command, args }` configuration
      contract on macOS, Windows, and Linux with the same three arguments and no
      Windows-only ASAR entry or `ELECTRON_RUN_AS_NODE` environment field.
