...
locator path. Every platform uses the exact public shape
`{ command, args: ['--pipilot-mcp-stdio', '--descriptor', descriptorPath] }`:

- macOS and installed Linux use the validated packaged executable;
- Windows uses the generated console-subsystem `PiPilot-mcp.exe` copy as a
  normal packaged Electron application. The copy loads `resources/app.asar`
  through Electron's application boundary; the public args do not contain an
  ASAR entry and the public configuration has no environment field;
