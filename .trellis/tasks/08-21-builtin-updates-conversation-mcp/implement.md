...
- [ ] Add platform-aware stable packaged command resolution, including validated
      `APPIMAGE` handling and fail-closed unpackaged behavior. Publish exactly
      `{ command, args }` on every platform, with args fixed to
      `--pipilot-mcp-stdio --descriptor <absolute-path>`; keep the Windows CUI
      executable selection inside packaging and expose no ASAR/env branch.
