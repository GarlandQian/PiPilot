...
External Control package evidence must install and launch the exact copied
`pipilot-mcp` command,
...
The copied server entry must be exactly `{ command: "pipilot-mcp", args: [] }`
on every platform; descriptor and bootstrap arguments remain Main-private.
Windows native packaged evidence must launch the generated lowercase
`pipilot-mcp.exe` through that contract and remains the authority for CUI stdio
behavior; static resolution and macOS tests do not substitute for it.
