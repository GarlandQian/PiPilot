...
### Runtime SDK Event Projection Boundary

When an official SDK event crosses Utility → Main → Renderer, distinguish the
event payload contract from process health:

- Decode the SDK event once in `projectRuntimeEvent()` and validate the shared
  `LocalPiRpcEvent` DTO. Preserve official JSON semantics such as omitted
  `undefined` tool `details`.
- If one non-transport event cannot be projected, emit only the bounded fixed
  `runtime_diagnostic/RUNTIME_EVENT_PROJECTION_FAILED` event, mark renderer
  state as sequence-uncertain, and refresh the authoritative snapshot.
- Reserve Host-fatal handling for Utility exit, MessagePort/transport failure,
  Host bootstrap failure, and explicit extension shutdown. Never use a single
  presentation parse error as evidence that every Runtime in the Host is dead.

Required regression path:

```
official write result (details: undefined)
  → DTO projection omits details
  → tool_execution_end reaches presentation as complete
  → Runtime remains ready and selected
  → next prompt is accepted
```

Test this path through the shared projector, Electron SDK fixture, and the
packaged smoke workflow; unit tests alone do not prove the embedded boundary.
