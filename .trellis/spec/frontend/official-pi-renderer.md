...
ComposerEditorHandle.insertMentionAtSelection(
  candidate: ComposerPathMentionCandidate,
...
- The Inspector Files tree may offer Add to Composer only for a ready project
  conversation. Re-project the Main-owned relative tree entry through
  `projectComposerMentionCandidates`; never construct mention attributes from
  arbitrary DOM text or add renderer filesystem access. Deliver the trusted
  candidate as a monotonically sequenced one-shot request bound to the exact
  project/session/runtime scope. Composer consumes every observed sequence once
  and ignores a scope mismatch so returning to an older scope cannot replay it.
  Insertion uses the same atomic node, canonical-path deduplication, validation,
  and Markdown serialization as typed `@`. Natural context-menu blur may retain
  an unchanged active `@query` range for replacement; explicit Escape, document
  edits, caret movement, or scope reset invalidate that replacement target.
  Ordinary-caret insertion preserves surrounding text and inserts separators as
  needed. The action returns focus to Composer but never expands a directory,
  opens a file preview, submits a prompt, or changes the active Session.
- Files-tree context actions support pointer invocation plus the platform
  context-menu key and Shift+F10. The menu item has localized accessible text,
  stays collision-bounded inside the window, and returns focus to Composer only
...
- PiPilot-owned queued messages retain their submitted text and image DTOs next
  to Pi's authoritative queue text. The queue surface shows bounded image
  previews and may promote an individual Follow-up to Steer only while every
  queued item has an unambiguous locally-owned payload. Promotion preserves all
  text, images, relative order, and queue modes. Reconnected, externally-owned,
  or duplicate-ambiguous queue rows fail closed: keep them executable, but do
  not offer a lossy promotion. Image-only Follow-up/Steer submission is rejected
  beside the Composer while preserving the draft and attachments.
...
- Sidebar Runtime markers use the same opaque selection identity when present;
  a duplicate `sessionId` without an exact token match must show no fabricated
  state. Running covers prompt acceptance, queued work, tool execution, retry,
  compaction, summarization, and extension interaction. Idle is completed unless
  the last assistant turn settled with an error stop reason, which is failed.
  Streamed token deltas must not republish the whole Runtime status snapshot.
...
| A Files-tree insertion request belongs to a replaced project/session/runtime scope | Consume the sequence and ignore it; never replay it when that scope is selected again |
| Files-tree insertion follows an unchanged active `@query` | Replace that exact query with the trusted atom; natural menu blur must not append `@query@path` |
| The user explicitly dismissed the query, edited its range, or moved the caret | Do not delete the old query; insert at the current selection with safe separators and canonical-path deduplication |
| A file or directory row opens its context menu | Do not run the row's ordinary click behavior, open a preview, expand a directory, or submit the Composer |
...
- Good: a successful official `write` result with `details: undefined` reaches
  the renderer as a valid completed tool call after JSON projection omits the
  undefined field; the selected Session remains ready and a later prompt is
  accepted.
- Good: `runtime_diagnostic/RUNTIME_EVENT_PROJECTION_FAILED` marks the
  projector as sequence-uncertain and triggers an authoritative snapshot; it
  does not clear the transcript or synthesize a Host crash.
...
- Good: right-clicking `src/App.tsx` while the Composer contains `review now`
  inserts the trusted atom at the saved caret and serializes exactly as
  `review [@src/App.tsx](src/App.tsx) now`; repeating the action changes no text.
...
- Bad: require `details` on every tool result or interpret one failed event
  projection as a Host crash. Both behaviors can make an otherwise persisted
  Session appear empty and permanently unselectable.
- Bad: append a plain `@path` string from the Files row, keep an insertion
  request after its scope changes, or reuse an explicitly dismissed `@query`.
...
- Files-tree Composer Electron coverage asserts pointer, ContextMenu-key, and
  Shift+F10 invocation; directory/file candidates; unchanged active-query
  replacement; Escape and caret-movement safety; duplicate stability; focus
  return; no row activation, preview, expansion, or prompt side effect; and the
  exact outgoing Markdown serialization.
...
const pathCandidate = projectComposerMentionCandidates([treeEntry], []).files[0]
if (pathCandidate) {
  enqueueComposerMentionInsertion({
    candidate: pathCandidate,
    scopeKey: activeComposerScopeKey,
    sequence: nextMentionInsertionSequence(),
  })
}
...
## 8. Scenario: Streaming tool-value projection

### 1. Scope / Trigger

Apply this whenever official Pi emits `toolcall_delta`, tool result content, or
a completed structured `LocalPiToolCall.arguments` value.

### 2. Signatures

- `presentToolCall({ argsPresentation?, resultPresentation?, ... })`
- `argsPresentation/resultPresentation: 'structured' | 'plain-text'`

### 3. Contracts

- `toolcall_delta.delta` is incremental raw JSON text and may be syntactically
  incomplete. Project it as bounded plain text until the authoritative completed
  tool call arrives.
- Text extracted from official tool-result `content` remains plain display text,
  even when it starts with `{` or `[`. Structured `details` stays structured.
- Completed official arguments and explicitly structured extension values keep
  using the bounded structured projector with truthful malformed state.

### 4. Validation & Error Matrix

| Input | Required presentation |
| --- | --- |
| Incremental `{"command":` delta | Plain text, running, not malformed |
| Official result text `{"partial":` | Plain text, not malformed |
| Completed malformed structured arguments | Truthful malformed structured value |
| Deep/large structured value | Existing bounded/truncated structured fallback |

### 5. Good / Base / Bad Cases

- Good: streaming arguments grow as readable text, then the completed call uses
  the appropriate specialized or structured presenter.
- Base: ordinary completed arguments/results retain current presentation.
- Bad: parse every string that resembles JSON, or suppress a real malformed
  completed value by marking all strings as plain text.

### 6. Tests Required

- Project an incomplete `toolcall_delta` and assert running text with
  `malformed: false`.
- Project incomplete-looking official result text and assert it remains text.
- Keep a control test proving completed malformed structured arguments still
...
```ts
projectStructuredValue(delta)
```

Correct:

```ts
projectPlainText(delta)
```
