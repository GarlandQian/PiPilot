...
the bounded `missing | installed | repair | unsupported` launcher state,
token-free portable command/args, authenticated-client count, and bounded
metadata-only recent rows. Installation is an explicit inline action; a managed
installed launcher may be removed only through an AlertDialog confirmation.
Show operating/errors beside the action and do not use a success dialog or
global toast. An available unowned launcher has no removal action. Main provides
an optional conversation label; components never format a raw conversation/
operation ID or fabricate a client label.
...
caption wrapping; the copied JSON uses the existing compact mono code surface.
Disable with authenticated clients requires the concise destructive
confirmation.
