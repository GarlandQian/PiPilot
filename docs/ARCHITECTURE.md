...
and a protocol/instance identity. The copied client configuration contains
only the packaged executable and descriptor path; it never contains the token.
...
The packaged command invokes the app executable directly with
`--pipilot-mcp-stdio --descriptor <absolute-path>`. On Windows the package also
contains `PiPilot-mcp.exe`; the copied MCP configuration passes the ASAR main
entry and `ELECTRON_RUN_AS_NODE=1`, so the launcher runs the MCP bootstrap as a
real Node stdio process instead of relying on a GUI-subsystem pipe. macOS and
Linux continue to use their packaged executable directly. Headless bootstrap
imports the MCP module without GUI creation, sets macOS activation policy to
`prohibited`, and injects the build version into MCP serverInfo. The regular
Electron path exits through `app.exit(code)`; the Windows Node-mode path sets
`process.exitCode` and drains stdio normally. Stdout is exclusively MCP JSON-RPC;
bounded unavailable/startup errors go to stderr and exit nonzero. It never
opens a BrowserWindow, TCP listener, Keychain capability token, or Renderer IPC.
