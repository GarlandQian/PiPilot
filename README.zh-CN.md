...
Integrations”现有标签中启用。启用后，PiPilot 会显示可复制的打包可执行文件命令和
descriptor 参数；能力 Token 只保存在本机 descriptor 中，不会进入复制的配置。
...
stdio 会返回有界的 unavailable 错误。
复制项采用通用形态
`PiPilot --pipilot-mcp-stdio --descriptor <descriptor>`；应使用已安装应用显示的精确
命令与参数，不要自行修改路径。
