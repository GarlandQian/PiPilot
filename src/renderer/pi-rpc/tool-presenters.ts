...
function legacyArgumentBody(input: ToolPresentationInput, kind: ToolCall['kind']) {
  if (kind === 'shell') {
  ...
  return input.args === undefined ? '' : projectStructuredValue(input.args).copyText
}
...
function genericPresentation(input: ToolPresentationInput): ToolCall {
  ...
  const projectedResult = input.resultText || input.resultDetails !== undefined
    ? projectStructuredValue(resultValue(input))
    : undefined
  ...
  const status = input.phase === 'queued'
    ...
    : input.phase === 'running'
      ...
      : input.isError
        ? 'failed'
        : 'success'
  const body = input.fallback?.body ?? legacyArgumentBody(input, kind)
  return {
    id: input.id,
    kind,
