...
export function PiRpcProvider({ children }: { children: React.ReactNode }) {
  ...
  }, [activeScopeKey, commitHydration, commitProjection, commitResponseActivities, commitSession])
  ...
  const runCommand = React.useCallback(async <TCommand extends LocalPiRendererRpcCommand,>(
    ...
    try {
      ...
      if (!expectedGeneration || runtimeRef.current?.state !== 'ready') {
      ...
      if (!changesSession && runtimeRef.current?.generation !== expectedGeneration) {
  ...
  }, [captureResponseActivity, settleResponseActivities])
