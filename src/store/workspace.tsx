...
export function WorkspaceProvider({ children }: { children: React.ReactNode }) {
  ...
  const removeWorkspace = React.useCallback(async (workspaceId: string) => {
    if (!adapter) return
    const removedScope = { kind: 'project', workspaceId } as const
    const result = await adapter.workspace.remove(workspaceId)
    const catalogKey = conversationScopeKey(removedScope)

    invalidateCatalogRequest(removedScope)
    applyWorkspaceSnapshot(result.snapshot)
    if (result.activeRemoved) applyActivation(result.activation)
    updateState((previous) => {
      const sessionCatalogs = { ...previous.sessionCatalogs }
      delete sessionCatalogs[catalogKey]
      return {
        ...previous,
        sessionCatalogs,
        errorCode: null,
    ...
    if (result.activeRemoved) {
      void refreshConversation(result.activation.scope, true).catch(fail)
    }
  }, [
    adapter,
    applyActivation,
