...
export interface ExternalControlState {
  ...
  launcher: ExternalControlLauncherSnapshot | null
  launcherOperation: 'install' | 'uninstall' | null
  launcherErrorAction: 'load' | 'install' | 'uninstall' | null
  launcherErrorMessage: string | null
}
...
export interface ExternalControlStoreValue extends ExternalControlState {
  ...
  installLauncher(): Promise<ExternalControlLauncherSnapshot | null>
  uninstallLauncher(): Promise<ExternalControlLauncherSnapshot | null>
  retryLauncher(): Promise<ExternalControlLauncherSnapshot | null>
...
export function ExternalControlProvider({ children }: { children: React.ReactNode }) {
  ...
  const [state, setState] = React.useState<ExternalControlState>(() => ({
    ...
    launcherErrorAction: adapter ? null : 'load',
    launcherErrorMessage: adapter ? null : 'The PiPilot desktop bridge is unavailable.',
  }))
  const stateRef = React.useRef(state)
  const requestEpoch = React.useRef(0)
  const launcherRequestEpoch = React.useRef(0)
  ...
  React.useEffect(() => {
    ...
    void adapter.getLauncher().then((launcher) => {
      if (!active) return
      update({ launcher, launcherErrorAction: null, launcherErrorMessage: null })
    }).catch((error) => {
      if (!active) return
      update({ launcherErrorAction: 'load', launcherErrorMessage: errorMessage(error) })
    })
    return () => {
      active = false
      unsubscribe()
  ...
  const retryLauncher = React.useCallback(async () => {
    if (!adapter) return null
    const epoch = ++launcherRequestEpoch.current
    try {
      const launcher = await adapter.getLauncher()
      if (epoch === launcherRequestEpoch.current) {
        update({ launcher, launcherErrorAction: null, launcherErrorMessage: null })
      }
      return launcher
    } catch (error) {
  ...
  const installLauncher = React.useCallback(async () => {
    if (!adapter) return null
    const epoch = ++launcherRequestEpoch.current
    update({
      launcherOperation: 'install',
      launcherErrorAction: null,
      launcherErrorMessage: null,
    })
    try {
      const launcher = await adapter.installLauncher()
      if (epoch === launcherRequestEpoch.current) {
        update({
          launcher,
          launcherOperation: null,
      ...
      if (epoch === launcherRequestEpoch.current) {
        update({
          launcherOperation: null,
          launcherErrorAction: 'install',
          launcherErrorMessage: errorMessage(error),
        })
      }
      return null
    }
  ...
  const uninstallLauncher = React.useCallback(async () => {
    if (!adapter) return null
    const epoch = ++launcherRequestEpoch.current
    update({
      launcherOperation: 'uninstall',
      launcherErrorAction: null,
      launcherErrorMessage: null,
    })
    try {
      const launcher = await adapter.uninstallLauncher()
      if (epoch === launcherRequestEpoch.current) {
        update({
          launcher,
          launcherOperation: null,
  ...
  const value = React.useMemo<ExternalControlStoreValue>(() => ({
    ...
    retry,
    uninstallLauncher,
  }), [installLauncher, retry, retryLauncher, setEnabled, state, uninstallLauncher])
