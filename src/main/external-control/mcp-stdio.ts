...
export function parseConversationMcpDescriptorPath(
  argv: string[],
  defaultDescriptorPath?: string,
  requireNoArguments = false,
) {
  if (requireNoArguments && argv.length > 0) {
    throw new ExternalControlError(
      'invalid_state',
      'The PiPilot MCP launcher does not accept arguments.',
    )
  }
  const indexes = argv.flatMap((value, index) =>
    value === DESCRIPTOR_FLAG ? [index] : [])
  if (indexes.length === 0 && defaultDescriptorPath) {
    if (
      defaultDescriptorPath.includes('\0') ||
      !isAbsolute(defaultDescriptorPath)
    ) {
      throw new ExternalControlError(
        'invalid_state',
...
export async function runConversationMcpStdio(
  ...
  if (!options.input) sourceInput.pipe(input)
  const discardInput = () => {
    if (!options.input) sourceInput.unpipe(input)
    sourceInput.pause()
    input.destroy()
  }
  let client: ConversationMcpBridgeClient | null = null
  try {
    const descriptorPath = parseConversationMcpDescriptorPath(
      argv,
      options.descriptorPath,
      options.requireNoArguments,
    )
