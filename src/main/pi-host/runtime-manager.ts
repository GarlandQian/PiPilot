...
} from './runtime-message-sanitizer'
...
export class RuntimeManager {
  ...
  constructor(options: RuntimeManagerOptions) {
    ...
    const callerExtensionsOverride = resourceLoaderOptions?.extensionsOverride
    this.resourceLoaderOptions = {
      ...
      extensionsOverride: (base) => {
        const overridden = callerExtensionsOverride?.(base) ?? base
        const sanitizer = base.extensions.find(
          ({ path }) => path === PIPILOT_RUNTIME_MESSAGE_SANITIZER_EXTENSION_PATH,
        )
        if (!sanitizer) return overridden
        return {
          ...overridden,
          extensions: [
            ...overridden.extensions.filter(
              ({ path }) => path !== PIPILOT_RUNTIME_MESSAGE_SANITIZER_EXTENSION_PATH,
            ),
            sanitizer,
          ],
    ...
    entry.unsubscribeSession = entry.runtime.session.subscribe(
      (event: AgentSessionEvent) => {
        try {
          ...
          // One presentation-event defect is not evidence that the shared Host
          // or its other Session runtimes are corrupt.
          this.emitRuntimeEvent(runtimeId, entry, {
            type: 'runtime_diagnostic',
            code: LOCAL_PI_RUNTIME_EVENT_PROJECTION_FAILED_CODE,
          })
        }
      },
    )
