...
import { resolveExternalControlMcpConfiguration } from './external-control/command-resolver'
    ...
    .then(async () => {
      ...
      const externalControlConfiguration = resolveExternalControlMcpConfiguration({
        ...
        ...(app.isPackaged && process.platform === 'win32'
          ? {
              mcpEntryPath: join(app.getAppPath(), 'out', 'main', 'index.js'),
              mcpExecutablePath: join(dirname(process.execPath), 'PiPilot-mcp.exe'),
            }
          : {}),
        isPackaged: app.isPackaged,
        ...(!app.isPackaged && process.env.PIPILOT_E2E_EXTERNAL_CONTROL_EXECUTABLE
          ? {
              testExecutablePath: process.env.PIPILOT_E2E_EXTERNAL_CONTROL_EXECUTABLE,
            }
          : {}),
      })
