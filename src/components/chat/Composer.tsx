      ...
      <ul className="flex flex-col gap-1">
        {values.map((value, index) => (
          <li
            key={`${index}:${value}`}
            title={value}
            className="truncate rounded-sm bg-muted/60 px-2 py-1 text-caption text-foreground"
          >
            {value}
          </li>
        ))}
    ...
    <Popover>
      ...
      >
        <div className="scroll-slim flex max-h-48 flex-col gap-2 overflow-y-auto">
          {queue.detailsKnown ? (
            <>
              <QueueList label={t('composer.steering')} values={queue.steering} />
              <QueueList label={t('composer.followUp')} values={queue.followUp} />
            </>
          ) : (
      ...
      <div className="mx-auto min-w-0 w-full max-w-[920px]">
        ...
        >
          ...
          <div
            ...
            className="flex min-h-10 min-w-0 items-center gap-0.5 px-2 pb-2 pt-0.5"
          >
            <input
              ref={fileInput}
              type="file"
            ...
            />
            <QueuePopover queue={queue} onSetQueueMode={onSetQueueMode} />
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="accent"
                        size="icon-xs"
                        className="h-8 w-7 rounded-none border-l border-sage-foreground/20"
                        disabled={!hasSubmission || submitting || Boolean(submissionConflict)}
