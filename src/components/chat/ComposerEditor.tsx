...
import { AllSelection, EditorState, PluginKey, TextSelection } from '@tiptap/pm/state'
import {
  EditorContent,
  NodeViewWrapper,
...
function insertTrustedMention(
  ...
  return editor.commands.command(({ dispatch, state, tr }) => {
    ...
    if (options.trigger) {
      ...
      if (!selectedText.startsWith(options.trigger)) return false
    }
    ...
    let from = tr.mapping.map(range.from)
    ...
    const insertionPosition = tr.doc.resolve(from)
    const previous = insertionPosition.nodeBefore
    const needsLeadingSeparator = options.trigger === undefined &&
      insertionPosition.parentOffset > 0 &&
      previous?.type.name !== 'hardBreak' &&
      !(previous?.isText && /\s$/u.test(previous.text ?? ''))
    ...
    if (options.validateDocument && !options.validateDocument(tr.doc.toJSON())) {
      tr.setMeta('preventDispatch', true)
      return false
    }
...
export const ComposerEditor = React.forwardRef<ComposerEditorHandle, ComposerEditorProps>(
  function ComposerEditor({
    ...
    const lastSuggestion = React.useRef<ComposerEditorSuggestion | null>(null)
      ...
      onTransaction({ editor, transaction }) {
        if (transaction.docChanged) {
          ...
          if (suggestion) {
            ...
            lastSuggestion.current = updatedSuggestion
    ...
    React.useImperativeHandle(forwardedRef, (): ComposerEditorHandle => ({
      ...
      dismissSuggestion() {
        if (activeSuggestion.current) exitSuggestion(editor.view, mentionPluginKey)
      ...
      insertMention(candidate, target, validateDocument) {
        ...
        return insertTrustedMention(editor, target, candidate, {
          trigger: '@',
          validateDocument,
        })
      },
      insertMentionAtSelection(candidate, validateDocument) {
        const { selection } = editor.state
        const suggestion = activeSuggestion.current ?? lastSuggestion.current
        if (
          suggestion &&
          revision.current === suggestion.documentRevision &&
          selection.empty &&
          selection.from === suggestion.to &&
          ...
          return insertTrustedMention(editor, suggestion, candidate, {
            trigger: '@',
            validateDocument,
          })
        }
        const { from, to } = selection
        return insertTrustedMention(editor, { from, to }, candidate, {
          validateDocument,
        })
      },
      removeMentions() {
      ...
      replaceLeadingSlashWithMention(candidate) {
        ...
        return insertTrustedMention(
          editor,
          { from: 1, to: 1 + text.length },
          candidate,
          { trigger: '/' },
        )
      },
