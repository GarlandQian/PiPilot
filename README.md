...
Settings > Integrations tab. When enabled, PiPilot shows a copyable command
and descriptor argument for the packaged executable; the capability token is
stored only in the local descriptor and is never placed in the copied config.
...
credential; a stopped or disabled app returns a bounded unavailable error.
The copied entry has the portable shape
`PiPilot --pipilot-mcp-stdio --descriptor <descriptor>`; use the exact command
and arguments shown by the installed application rather than editing the path.
